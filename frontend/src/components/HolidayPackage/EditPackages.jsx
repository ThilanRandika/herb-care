...
const EditPackages = () => {
  ...
  const handleDeletePackage = (pkgId) => {
    setDeletePackageId(pkgId); // Set the package ID to be deleted
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8070/packages/${deletePackageId}`); // Send delete request to backend
  ...
  return (
    <Container>
      <h2 className="mb-4">Edit Packages</h2>
      <Row>
        {packages.map((pkg) => (
